...
- **Latest-rate semantics and caching:** without a type filter, `GET /rates/latest` will return one current record for each provider + rate-type pair. With `?type=`, it will return one current record per provider for that type. “Latest” is ordered by effective date, then ingestion timestamp, then record ID, all descending. Redis will cache the response and a successful ingest will invalidate only the affected latest-rate cache key(s); the exact key format will be recorded when implemented.
